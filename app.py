...
def save_regions():
    ...
    for r in data:
        ...
        if name:
            db.execute("INSERT INTO regions (name, sort_order) VALUES (?,?)", (name, sort_order))
    db.commit()
    rows = db.execute("SELECT * FROM regions ORDER BY sort_order, id").fetchall()
    return jsonify([dict(r) for r in rows])
...
def save_estimators():
    data = request.json or []
    if not isinstance(data, list):
        return jsonify({"error": "Expected a list of estimators"}), 400
    db = get_db()
    db.execute("DELETE FROM estimators")
    for e in data:
        name = (e.get("name") or "").strip()
        color = e.get("color") or "#3b82f6"
        region_id = int(e.get("region_id") or 1)
        active = 1 if e.get("active", True) else 0
        sort_order = int(e.get("sort_order") or 0)
        if name:
            db.execute(
                "INSERT INTO estimators (name,color,region_id,active,sort_order) VALUES (?,?,?,?,?)",
                (name, color, region_id, active, sort_order),
            )
    db.commit()
    rows = db.execute(
        "SELECT e.*, r.name AS region_name FROM estimators e JOIN regions r ON e.region_id=r.id "
...
def create_booking():
    ...
    if RFMS_ENABLED:
        # 1) find customer
        cid, err = rfms_find_customer_id_by_phone(phone)
        if err:
            rfms_log.append("RFMS customer find failed: " + err)
        elif cid:
            rfms_customer_id = cid
